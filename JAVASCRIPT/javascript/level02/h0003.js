...
btnOk3.addEventListener('ckick', function() {
  var username = txtUsername.value.replace(/\s/g, '');
  // if elself else 这个是阶梯式判断
  // if(逻辑表达1){逻辑表达式1为真要执行的代码}
  // else if (逻辑表达式2) {逻辑表达式1为真要执行的代码}
  // .....
  // else {所有上面的表达式都不为真就执行这里面的代码}
  // 阶梯判断是从上到下一次判断，任何一级判断为true，后面都不执行了
  if (username.length < 8) {
...
console.log(divInput, divStyle, txtNum, spNum);
var count = 0;
divStyle.addEventListener('click', function() {
  ...
  if (fnum == inum) {
    ...
    if(inum>=100 && inum <= 200) {
      console.log((spNum.innerHTML = '奇怪的奇数出现了'));
    }
    // if(!(inum<100 || inum>200)) 
  } else {
