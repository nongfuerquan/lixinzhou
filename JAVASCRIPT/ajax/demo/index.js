...
btnOne.addEventListener('click', function () {
  ...
  huhuiyu.ajax('POST', '/test/echoOne', param, function (resp) {
    // 应答结果固定值：code,message,resultDate,success,token
    // 文档里面有说明
    console.log(resp);
    divResp.innerHTML = JSON.stringify(resp);
    // 获取应答的json中的数据
    var divEchoOne = document.getElementById('divEchoOne');
    divEchoOne.innerHTML = resp.message + '###' + resp.random;
  });
});
...
btnThree.addEventListener('click', function () {
  ...
  huhuiyu.ajax('POST', '	/test/echoThree', param, function (resp) {
    console.log(resp);
    divResp.innerHTML = JSON.stringify(resp);
    var divEchoThree = document.getElementById('divEchoThree');
    divEchoTwo.innerHTML = 
    'pageNumber:' + resp.resultData.Data.page.pageNumber;
    'pageSize:' + resp.resultData.Data.page.pageSize;
    'nickname:' + resp.resultData.Data.page.nickname;
    'username:' + resp.resultData.Data.page.username;
  });
});
